...
const Contact = () => {
  ...
  // idle | sending | success | error | mailto  ('mailto' = handed off to the
  // visitor's mail client because EmailJS is not configured — not a send)
  ...
  const handleSubmit = async (e) => {
    ...
    const consented = form.querySelector('#permission')?.checked;
    ...
    if (!consented) {
      setErrorMsg('Please tick the permission box so I can reply to you.');
      setStatus('error');
      setTimeout(() => setStatus('idle'), 4000);
      return;
    }
    ...
    if (!isConfigured) {
      // EmailJS is not configured, so nothing can be sent from the page. Hand off
      // to the visitor's mail client — but we cannot confirm delivery, so don't
      // claim success and don't clear the form (their message would be lost).
      ...
      <div className="relative z-10 w-full flex justify-end items-end">
        ...
        >
          <div className="flex flex-col sm:flex-row justify-between items-start gap-8 mb-12">
            ...
            >
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M1.5 8.67v8.58a3 3 0 003 3h15a3 3 0 003-3V8.67l-8.928 5.493a3 3 0 01-3.144 0L1.5 8.67zM22.5 6.908V6.75a3 3 0 00-3-3h-15a3 3 0 00-3 3v.158l9.714 5.978a1.5 1.5 0 001.572 0L22.5 6.908z" /></svg>
              Email Me
            </a>
          </div>
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col gap-12 md:gap-16 w-full">
            ...
            <div className="flex flex-col md:flex-row gap-12 mt-4">
              ...
              <div className="flex-1 flex flex-col gap-8 text-xs text-white/70 font-medium">
                ...
                {/* Status feedback — announced to screen readers too */}
                <p aria-live="polite" className="min-h-[1.25rem] text-xs font-bold">
                  {status === 'error' && <span className="text-black">{errorMsg}</span>}
                  {status === 'success' && (
                    <span className="text-black">Thanks — your message is on its way to my inbox.</span>
                  )}
                  {status === 'mailto' && (
                    <span className="text-black">
                      Your email app should open with the message ready — press send there to reach me.
                    </span>
                  )}
                </p>
